...
var inputManagerOperations = {
    Vector1: (leftSide, rightSide, chosenFunction) => {
        ...
        return new Vector((chosenFunction(leftSide)?-1:0) + (chosenFunction(rightSide)?1:0))
    },
    Vector2: (leftSide, rightSide, upSide, downSide, chosenFunction) => {
        chosenFunction = (chosenFunction == undefined)?onKey:chosenFunction;
        return  new Vector(
            (chosenFunction(leftSide)?-1:0) + (chosenFunction(rightSide)?1:0),
            (chosenFunction(upSide)?-1:0) + (chosenFunction(downSide)?1:0)
        )
    },
    Vector3: (leftSide, rightSide, upSide, downSide, forward, backward, chosenFunction) => {
        ...
        return  new Vector(
            (chosenFunction(leftSide)?-1:0) + (chosenFunction(rightSide)?1:0),
            (chosenFunction(upSide)?-1:0) + (chosenFunction(downSide)?1:0),
            (chosenFunction(forward)?-1:0) + (chosenFunction(backward)?1:0)
        )
    }
}
